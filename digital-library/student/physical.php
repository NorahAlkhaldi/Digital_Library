        ...
        ?>
          <div class="col-xl-3 col-md-6 d-flex align-items-stretch" data-aos="zoom-in" data-aos-delay="100">
            <div class="icon-box">
              ...
              <h4><a href="resource_details.php?id=<?= $data['id'] ?>"><?= $data['name'] ?></a></h4>
              <p style="max-height: 75px;overflow-y: hidden;"><?= $data['description'] ?></p>
              <hr>
              <a class="btn btn-primary btn-sm" href="borrow_resource.php?id=<?= $data['id'] ?>&title=<?= $data['name'] ?>" <?php if($data['status']==1){ ?> disabled <?php } ?> >Book Resource</a>
            </div>
          </div>
        <?php } ?>
        </div>
